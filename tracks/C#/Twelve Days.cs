...
public static class TwelveDays
{
    ...
    {
        return $"On the {All[verseNumber].Item1} day of Christmas my true love gave to me: {All.Take(verseNumber).Reverse().Aggregate(new StringBuilder(), (x, y) => x.Append(y.Value.Item2))}.";
    }
    public static string Recite(int startVerse, int endVerse)
    {
        var result = new StringBuilder();
        for (int i = startVerse; i <= endVerse; i++) result.Append(Recite(i) + '\n');
        return result.ToString().Trim('\n');
    }

    static SortedDictionary<int, Tuple<string, string>> All = new SortedDictionary<int, Tuple<string, string>>()
    {
